...
from utils.utils import get_close_assignment, get_model_name, get_checkpoint_path, get_data_path, get_log_path
from sat_cls import clause_satlit_count

# training for supervised clause brittleness tier predictor (CTP)
...
# save clause embeddings with their ground truth tier labels (0, 1, 2) for training the clause brittleness predictor
...
def save_labelled_clauses_embedding(model_name, test_data="SR(U(10,40))", save_name=""):
    ...
    for i, (clauses, n_vars, is_sat_ground_truth) in enumerate(data):
        ...
        save_instance(f"clause_emb_{save_name}.h5", i, C_h, vote, sat_lit_count)

        if (i+1) % 100 == 0:
            print(f"Processed {i+1}...")


# Clause Tier Predictor Model + Training 
    ...
    def __init__(self, h5_file):
        ...
        with h5py.File(h5_file, 'r') as f:
            for inst_id in f.keys():
                ...
                # Pre-process features (65D) and brittleness tiers (0, 1, 2)
...
class ClauseTierPredictor(nn.Module):
    def __init__(self, dimension=128, dropout=0.2):
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(64, 3) # Output: [0:Brittle, 1:Medium, 2:Slack]
        )
...
def auto_hyperparameter_tuning_ctp():
    data_loader = DataLoader(ClauseDataset("clause_emb_train.h5"), batch_size=1024, shuffle=True)
    val_loader = DataLoader(ClauseDataset("clause_emb_val.h5"), batch_size=1024, shuffle=False)
